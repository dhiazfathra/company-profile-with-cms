...
caller *declares* for it, so a wrong declared size does not fail — it returns a
...
| `bun run test` | The guardrails' own tests — each failure mode reproduced |
...
import {
  // capture
  captureAll, captureTarget, recrop, validateConfig,
  // cropping and chrome
  findSelection, cropToSelection, cropRelative, healOutlines,
  findViewerChrome, assertNoViewerChrome, scanAssets,
  // fidelity
  loadManifest, checkSection, screenshotSection, awaitImages,
  ASPECT_TOLERANCE, BLOCK_TOLERANCE, DEFAULT_REFS_DIR,
} from 'figma-to-site'
```
